...
def arg_opts():
    ...
    parser.add_argument(
        "-b", "--button", metavar="button_id", dest="button", default=0, help="Button"
    )

    fixed_grp = parser.add_mutually_exclusive_group()

    fixed_grp.add_argument(
        "-f",
        "--fixed",
        metavar="fixed_code",
        dest="fixed",
    ...
    fixed_grp.add_argument(
        "-i", "--id", metavar="remote_id", dest="id", default=None, help="Remote-ID"
    )
...
def grok_json(args):
    ...
    for p in id_list.values():
        ...
        p_fixed = int(p["fixed"])
        print_file(
            p_rolling, p_fixed, fname=args.outfname, quiet=args.quiet, rf_freq=fr
        )
...
def main():
    ...
    if _debug:  # and (fixed_out or rolling_out):
        print(
            f">> rolling_out  {rolling_out:12d} "
            f"{rolling_out:010X} {rolling_out:016b}"
        )
        print(f">> fixed_out    {fixed_out:12d} " f"{fixed_out:010X} {fixed_out:040b}")
    ...
    if _debug:
        ...
        print(
            f"fixed_code {fixed_code:12d} {fixed_code:010X} {fixed_code:040b}"
        )  # noqa
